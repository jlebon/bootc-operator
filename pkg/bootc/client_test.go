...
func TestClientStatus(t *testing.T) {
	...
	runner.setHandler("bootc status --json", []byte(statusJSON), nil)
...
func TestClientIsBootcHost(t *testing.T) {
	...
	runner.setHandler("bootc status --json", []byte("{}"), nil)
	c := NewClientWithRunner(runner)
	...
	runner2.setHandler("bootc status --json", nil, fmt.Errorf("not found"))
...
func TestClientSwitch(t *testing.T) {
	runner := newFakeRunner()
	runner.setHandler("bootc switch", nil, nil)
...
func TestClientUpgradeDownloadOnly(t *testing.T) {
	runner := newFakeRunner()
	runner.setHandler("bootc upgrade --download-only", nil, nil)
...
func TestClientUpgradeApply(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			softReboot: false,
			wantArgs:   []string{"upgrade", "--from-downloaded", "--apply"},
		},
		{
			name:       "with soft reboot",
			softReboot: true,
			wantArgs: []string{
				"upgrade", "--from-downloaded", "--apply", "--soft-reboot=auto",
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			runner := newFakeRunner()
			runner.setHandler("bootc upgrade", nil, nil)
...
func TestClientRollback(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "without apply",
			apply:    false,
			wantArgs: []string{"rollback"},
		},
		{
			name:     "with apply",
			apply:    true,
			wantArgs: []string{"rollback", "--apply"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			runner := newFakeRunner()
			runner.setHandler("bootc rollback", nil, nil)
...
func TestClientStatusParseError(t *testing.T) {
	runner := newFakeRunner()
	runner.setHandler("bootc status", []byte("not json"), nil)
...
func TestClientStatusCommandError(t *testing.T) {
	runner := newFakeRunner()
	runner.setHandler("bootc status", nil, fmt.Errorf("exit status 1"))
